...
/*
 Bestand downloaden
*/
if(isset($_GET["downloadFileNews"])){
  if(valid($_SESSION["newsURL"])){
    $file_name = $_SESSION["newsURL"].$_GET['downloadFileNews'];
...
if(isset($_FILES["fileToUploadNews"])){
  $newName = $_POST["newNameNews"];
  $target_dir = $_SESSION["newsURL"];
  $target_file = $target_dir . $newName;
  $uploadOk = 1;
  if($_FILES["fileToUploadNews"]["size"] > 209715200){
    $uploadOk == 0;
  }
  // Check if $uploadOk is set to 0 by an error
  if ($uploadOk == 0) {
      echo "Sorry, bestand is niet geupload. Klik <a href='../newsletters.php'>hier</a> om terug te gaan";
  // if everything is ok, try to upload file;
  } else {
      if (move_uploaded_file($_FILES["fileToUploadNews"]["tmp_name"], $target_file)) {
          // log
          $sql = "INSERT INTO logs (logs_content) VALUES (:message);";
          $query = $conn->prepare($sql);
          $result = $query->execute(array(
           ":message" => "Bestand \"".$newName. "\" toegevoegd in nieuwsbrieven!"
         ));
      } else {
          echo "Sorry, er is een probleem tijdens het uploaden. Klik <a href='../newsletters.php'>hier</a> om terug te gaan";
      }
  }
